...
//
// IMPORTANTE:
// - Configurações do médico/clínica vêm agora de AgendaConfig (via getCabecalhoDocumento_/buildCabecalhoHtml_).
...
function receitaCriar_(payload) {
  ...
  var linha = [idReceita, idPaciente, dataHoraCriacao, textoMedicamentos, observacoes];
...
/**
 * Gera HTML da receita para impressão (PDF) com:
 * - Cabeçalho do médico + clínica + logo (via AgendaConfig / DocsCabecalho)
 * - Nome do paciente e CPF (se existir)
 *
 * payload:
 * {
...
function receitaGerarPdf_(payload) {
  ...
  // ==============================
  // CABEÇALHO (médico / clínica / logo) via DocsCabecalho + AgendaConfig
  // ==============================
  var cabecalhoHtml = buildCabecalhoHtml_();    // HTML pronto do cabeçalho
  var cab = getCabecalhoDocumento_();           // dados (para assinatura)

  // ==============================
  // BUSCA NOME E CPF DO PACIENTE
  // ==============================
  ...
  // Cabeçalho padrão (médico / clínica / logo)
  html += cabecalhoHtml;
 ...
 * Usa DocsCabecalho / AgendaConfig.
 *
 * payload: { idReceita }
 */
function receitaGerarPdf_(payload) {
  ...
  var cabecalhoHtml = buildCabecalhoHtml_();
  var cab = getCabecalhoDocumento_();
