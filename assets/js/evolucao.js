...
// Tela de Evolução Clínica
// - Carrega paciente atual (localStorage)
...
function setMensagemEvolucao(texto, tipo = "info") {
  const div = document.getElementById("mensagemEvolucao");
  if (!div) return;

  div.textContent = texto || "";

  div.classList.remove("mensagem-info", "mensagem-erro", "mensagem-sucesso");
  if (tipo === "erro") div.classList.add("mensagem-erro");
  else if (tipo === "sucesso") div.classList.add("mensagem-sucesso");
  else div.classList.add("mensagem-info");
}
...
function prepararModal() {
  ...
  const btnFechar = document.getElementById("btnFecharModalEvolucao");
...
function abrirModalEvolucao(idEvolucao) {
  ...
  if (spanData) {
    spanData.textContent = formatarDataParaBR(ev.dataEvolucao || ev.data || "");
  }
  if (spanTipo) {
...
function usarComoBaseNovaEvolucao() {
  ...
  if (textareaTexto) {
    textareaTexto.value = evolucaoModalAtual.texto || evolucaoModalAtual.resumo || "";
  }
...
async function carregarEvolucoes() {
  ...
  evolucoesCache =
    (resposta.data && resposta.data.evolucoes) ? resposta.data.evolucoes : [];
...
function renderizarTabelaEvolucoes(lista) {
  ...
  if (!lista || !lista.length) {
    if (msgLista) {
      if (!evolucoesCache.length) {
        msgLista.textContent = "Nenhuma evolução registrada para este paciente.";
      } else if (termo) {
  ...
  lista.forEach((ev) => {
    ...
    tdData.textContent = formatarDataParaBR(ev.dataEvolucao || ev.data || "");
...
function carregarEvolucaoParaEdicao(idEvolucao) {
  ...
  if (inputTipo)
    inputTipo.value = ev.tipoEvolucao || ev.tipo || "";
  if (inputProf)
    inputProf.value = ev.profissional || "";
  if (textareaTexto)
    textareaTexto.value = ev.texto || ev.resumo || "";
...
function confirmarInativarEvolucao(idEvolucao, evRef) {
  ...
  const data = formatarDataParaBR(evRef.dataEvolucao || evRef.data || "");
