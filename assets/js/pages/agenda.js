 ...
 *
 * Este módulo é carregado dinamicamente por main.js quando data-page-id="agenda".
 * Exporta: initAgendaPage()
 */
...
export function initAgendaPage() {
  ...
  async function carregarAgendaDia() {
    ...
    try {
      ...
      console.error(error);
      mostrarErro("Não foi possível carregar a agenda do dia: " + error.message);
    } finally {
  ...
  function atualizarResumoDia(resumo) {
    ...
    if (resumoConfirmadosEl) resumoConfirmadosEl.textContent =
      resumo?.confirmados ?? 0;
    if (resumoFaltasEl) resumoFaltasEl.textContent = resumo?.faltas ?? 0;
    if (resumoCanceladosEl) resumoCanceladosEl.textContent =
      resumo?.cancelados ?? 0;
    if (resumoConcluidosEl) resumoConcluidosEl.textContent =
      resumo?.concluidos ?? 0;
  ...
  function abrirProntuario(ag) {
    ...
    try {
      localStorage.setItem("prontio.pacienteSelecionado", JSON.stringify(info));
    } catch (e) {
      console.warn("Não foi possível salvar paciente no localStorage", e);
    }

    const url = "prontuario.html?idPaciente=" + encodeURIComponent(ag.ID_Paciente);
    ...
    try {
      ...
      if (mensagemNovoAgendamento) {
        if (error.code === "AGENDA_CONFLITO_BLOQUEIO") {
          ...
          let msg = "Não é possível agendar: já existe consulta neste horário.";
          if (horaIni && horaFim) {
